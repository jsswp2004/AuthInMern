...
export default function VisitList() {
  ...
  //#endregion
  //
  //#region code to start on a sunday for weekly view
  ...
  //#endregion
  //
  //#region code for filtering visits with dates / this method will filter the table daily
  const filteredData = visits.filter((el) => {
    //if no input the return the with the original default date
    if (searchInput === '') {
      // return el
      return Object.values(el).toString().toLowerCase().includes(dateSelected)
      //.includes(daySelected > startOfTheWeek
      // )
      // > startOfTheWeek && el.visitDate < startOfTheWeek + 6)
    }
    //return the item which contains the user input
      ...
      //.includes(
      //  daySelected > startOfTheWeek
      //)
      // > startOfTheWeek && dateSelected < startOfTheWeek + 6
    }
  })
  //#endregion
  //
  ...
  //#endregion
  //
  //#region This method fetches the visits from the database.
  useEffect(() => {
    async function getVisits() {
      const response = await fetch(`http://localhost:5002/visit/`)
  ...
  //#endregion
  //
  //#region This method will delete a visit
  async function deleteVisit(id) {
    await fetch(`http://localhost:5002/visit/${id}`, {
      method: 'DELETE',
    })

    const newVisits = visits.filter((el) => el._id !== id)
    setVisits(newVisits)
  }
  //#endregion
  //
  //#region This method will map out the visits on the table daily
  function visitList() {
    return [...filteredData]
      .sort((a, b) =>
  ...
  //#endregion
  //
  //#region these 'visitListWeekly' methods will map out the visits on the table weekly per day also sorts the time of the day
  ...
  //#endregion
  //
  //#region these 'visitListMonthly' methods will map out the visits on the table weekly per month
  function visitListMonthly() {
    ...
    return (
      [...filteredDataMonthly]
        ...
        //.filter((el) => moment(el.visitDate).format('d') === x)

        .map((visit) => {
          //const date = moment(visit.visitDate).format('D')
          // console.log(date)
          return (
            <VisitWeekly
              visit={visit}
        ...
        .filter((a, b) => moment(a.visitDate) !== moment(b.dateSelected))
    )
  }
  //#endregion
  //
  //#region montly basic dates values
  // const endOfTheMonth = moment(dateSelected).endOf() // Monday format f
  // const selectedDay = moment(dateSelected).format('dddd') // Monday format
  // const monthSelectedString = moment().month(dateSelected).format('MMMM') //string month format
  // const monthSelected = moment(dateSelected).month(dateSelected).format('MM') //month number format
  // const endOfTheMonth = moment(dateSelected).endOf('month').format('DD') // Monday format
  const endOfThePreviousMonth = parseInt(
    moment(dateSelected).subtract(1, 'months').endOf('month').format('DD'),
  ) // end of previous month
  // const startOfMonth = moment(dateSelected).startOf('November').format('DD');
  // const weekdate = moment(dateSelected).format('dd') //string short day format eg. 'Mo
  // const selectedDate = moment(dateSelected).format('DD') //number format '01'
  // const selectedYear = moment(dateSelected).format('YYYY') //number format '2020'
  // const weekDayStart = moment(
  //   selectedYear + '-' + monthSelected + '-' + firstsDayofMonth,
  // ).weekday()
  // let firstsDayofMonth2 = moment(newdate).startOf('month').format('DD')
  // const lastDayofMonth = moment(dateSelected)
  //   .subtract(1, 'months')
  //   .endOf('month')
  //   .format('DD')

  // let startDayofTheMonth = moment(dateSelected).startOf('month').date()

  // const gridColumnStart = {
  //   gridColumnStart: startDayofTheMonth,
  //   backgroundColor: ' #eeee',
  //   height: '120px',
  // }
  ...
  let startOfTheMonth = moment(dateSelected)
    .startOf('month')
    .format('YYYY-MM-DD')
  ...
  const monthlyDay1 = moment(startOfTheMonth)
    .add(0, 'days')
    .format('YYYY-MM-DD')

  const visitsMonthlyDay1 = visits.filter((el) => {
    //if no input the return the with the original default date
    if (searchInput === '') {
      // return el
      return Object.values(el).toString().toLowerCase().includes(monthlyDay1)
    }
    //return the item which contains the user input
    else {
      return Object.values(el).toString().toLowerCase().includes(monthlyDay1)
  ...
  //   visitsMonthlyDay1,
  //   monthlyDay1,
  //   moment(monthlyDay1).format('dddd'),
  //   'visitListMonthlyDay1',
  // )
  //#endregion
  //
  //#region second day of the month
  const monthlyDay2 =
  ...
  // console.log(moment(dateMonthly_01).add(1, 'days').format('YYYY-MM-DD'))
  // console.log(dateMonthly_02)
  ...
  return (
    <div className="grid-container">
      <div className="item1" style={{ height: 50 }}>
        <div className="div-items">{/* <h3>Schedule for </h3> */}</div>
            ...
            >
              ...
              <div>
                ...
                >
                  ...
                  <tbody className="trStyles">{visitList()}</tbody>
                </table>
              </div>
            </li>
          </ul>
